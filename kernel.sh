...
awk -F ')' '{print $2}' | awk '{print tolower($1)}')"
export ZIPNAME="${KERNELNAME}-0.1-${DEVICE}-TREBLE-$(date +%Y%m%d-%H%M).zip"
export FINAL_ZIP="${ZIP_DIR}/${ZIPNAME}"
[ ! -d "${ANYKERNEL}" ] && git clone --depth=1 https://github.com/VRanger/AnyKernel2 ${ANYKERNEL}
